    ...
    Scaffold(bottomBar = { MyBottomBar(navController1) }) { innerPadding ->
        NavHost(navController = navController1, startDestination = Routes.Home.routes,
            ...
            composable(Routes.Notification.routes) {
                Notification()  // Call the composable function for the Notification screen
            }
            composable(Routes.Search.routes) {
                Search()  // Call the composable function for the Search screen
            }
            composable(Routes.AddThreads.routes) {
                AddThreads(navController)  // Call the composable function for the AddThreads screen
            }
            composable(Routes.Profile.routes) {
                Profile(navController)  // Call the composable function for the Profile screen
            }
